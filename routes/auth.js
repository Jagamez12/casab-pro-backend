...
router.post('/register', async (req,res) => {
    ...
    try {
        ...
        const tokenMail = mailcont.getToken({email: user.email, name: user.name, time: Date.now()})
...
router.post('/recovery', async(req, res) => {
    ...
    if(!user){
        return res.status(400).json({error: "Something goes wrong!", redirect: false})
    }else{
        try {
            const tokenMail = mailcont.getToken({email:user.email, user: user.name})
            ...
            return res.status(200).json({success: true, redirect: true, msg: "Tu correo ha sido enviado"})
        } catch (error) {
            console.log(error)
            return res.status(500).json({success: false, redirect: false, msg: 'Something goes wrong'})
...
router.post('/change-password/:token', async (req, res) => {
    ...
    if(!user){
        ...
        if(newPassword !== rePassword){
            return res.status(400).json({error: 'the passwords are not similar, please try again', redirect: false})
        }else{
            try {
                const hashedPassword = await bcrypt.hash(newPassword, 10)
                user.password = hashedPassword
                await user.save()
                res.status(200).json({success:true, redirect: true, msg: 'Your password has been updated successfully'})
...
router.get('/recovery-password/:token', async(req, res)=>{
    ...
    if(!user){
        ...
        if(newPassword !== rePassword){
            return res.status(400).json({error: 'the passwords are not similar, please try again'})
        }else{
            try {
                const hashedPassword = await bcrypt.hash(newPassword, 10)
                user.password = hashedPassword
...
function createAccessToken(user){
    return mailcont.getToken({name: user.name, _id:user._id, email: user.email})    
}
